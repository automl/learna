...
def generate_validation_pipeline_file(config_id, config, job_id, mode, root_dir):
    ...
    with open(root_dir.joinpath('utils/validation_' + job_id + '_' + config_id + '.moab'), 'w') as validation_script:
        ...
        validation_script.write('tar -xzvf /work/ws/nemo/fr_ds371-learna-0/data/rfam_learn.tar.gz -C $TMPDIR/${MOAB_JOBID}\n')
        validation_script.write('\n')
        ...
        validation_script.write('  --save_path $TMPDIR/${MOAB_JOBID}/models/${MOAB_JOBARRAYINDEX}/ \\\n')
        ...
        for entry in config:
            if 'conv' in entry and not conv_written:
                conv_written = True
                validation_script.write(f"  --conv_sizes {config['conv_size1']} {config['conv_size2']} \\\n")
                validation_script.write(f"  --conv_channels {config['conv_channels1']} {config['conv_channels2']} \\\n")
                continue
            elif 'conv' in entry and conv_written:
                continue
            elif entry == 'restart_timeout':
                continue
            else:
                validation_script.write(f"  --{entry} {config[entry]} \\\n")
        ...
        validation_script.write('\n')
        validation_script.write('cp -r $TMPDIR/${MOAB_JOBID}/models/${MOAB_JOBARRAYINDEX}/' + f" {root_dir}/models/" + 'validation_${MOAB_JOBARRAYINDEX}_' + f"{job_id}_{config_id}")
